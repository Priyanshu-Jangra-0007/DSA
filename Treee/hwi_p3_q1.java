...
public class hwi_p3_q1{
    ...
    public static void build(int node,int l,int r,long[] arr){
        if(l==r){
            seg[node]=arr[l]%MOD;
            return;
        }
    ...
    public static long apSum(long len,long a,long d){
    ...
    public static void update(int node,int l,int r,int ql,int qr,long x,long y){
        if(qr<l||r<ql)return;
        if(ql<=l&&r<=qr){
            long a=(x+(long)(l-ql)*y%MOD)%MOD;
            seg[node]=apSum(r-l+1,a,y);
            lazyA[node]=a;
            lazyD[node]=y;
            hasLazy[node]=true;
            return;
